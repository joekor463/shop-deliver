...
const FilterControls = ({activeFilter, basePath, searchParams = {}}: FilterControlsProps) => {
    ...
    const filterButtonText = activeFilterCount === 0 
                        ? "Фильтры" 
                        : activeFilterCount === 1
                            ? "Фильтр 1" : `Фильтры ${activeFilterCount}` 
    

    return (
                    <div className="flex flex-row gap-x-6 mb-6">
                <div
                    className={`h-8 p-2 rounded text-xs flex justify-center
                    items-center duration-300 cursor-not-allowed gap-x-2 ${
                  ...
                  {filterButtonText} 
                   <div
                        className={`h-8 p-2 rounded text-xs flex justify-center
                    items-center duration-300  gap-x-2 ${
                        !activeFilter || activeFilter.length === 0 
                            ? "bg-[#f3f2f1] text-[#606060]  "
                            : "bg-(--color-primary) text-white"
                    }`}
                   >
                        <Link href={buildClearFilterLink()}
                              className="flex items-center gap-x-2"  
                        >Очистить фильтры
                            <Image src="/icon-products/icon-closer.svg" 
                            alt="Очистить фильтры" 
                            width={24} 
                            height={24}
                            style={!activeFilter || activeFilter.length === 0 ? {} : 
